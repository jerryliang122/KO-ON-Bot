...
def custom_joincommand(config: dict, botid: str, logger: Logger) -> str:
    try:
        ...
        logger.warning(f"自定义呼叫命令装载:{config[f'join_command{botid}']}")
...
def custom_preferred_platform(config: dict, botid: str, logger: Logger) -> str:
    try:
        assert f'default_platform{botid}' in config
        if config[f'default_platform{botid}'] in platform:
            logger.warning(f"自定义首选平台装载:{config[f'default_platform{botid}']}")
            return config[f'default_platform{botid}']
        else:
    ...
    return [
        {
            ...
            "modules": [
                {
                    "type": "header",
                    "text": {"type": "plain-text", "content": "点歌机操作指南"},
                },
                {"type": "divider"},
                {
                    "type": "section",
                ...
                {
                    "type": "section",
                    "text": {"type": "kmarkdown", "content": "**0.  退出语音**"},
                },
                {
                    "type": "section",
                    "text": {
                ...
                {
                    "type": "section",
                    "text": {"type": "kmarkdown", "content": "**6.  重新连接**"},
                },
                {
                    "type": "section",
                    "text": {
                ...
                {
                    "type": "section",
                    "text": {"type": "kmarkdown", "content": "**7.  循环模式**"},
                },
                {
                    "type": "section",
                    "text": {
                ...
                {
                    "type": "section",
                    "text": {"type": "kmarkdown", "content": "**8.  下一首**"},
                },
                {
                    "type": "section",
                    "text": {
                ...
                {
                    "type": "section",
                    "text": {"type": "kmarkdown", "content": "**9.  清空歌单**"},
                },
                {
                    "type": "section",
                    "text": {
                ...
                {
                    "type": "section",
                    "text": {"type": "kmarkdown", "content": "**10.  歌单**"},
                },
                {
                    "type": "section",
                    "text": {
...
async def getInformation(duration: dict, deltatime: int, bvid: str, guild: str,
    ...
    if len(bvid) == 12:
        ...
        cid, title, mid, name, pic = await getCidAndTitle(
            ...
            int(bvid[13:]))
        item.append(bvid[:12])
    item.extend((cid, title, mid, name, pic))
    return item
...
async def getAudio(guild: str, item: list, botid: str, bili_cookie:str,session: ClientSession):
    ...
    bvid, cid, title, mid, name, pic = item[0], item[1], item[2], item[
        3], item[4], item[5]
    url = f'{baseUrl}bvid={bvid}&cid={cid}'
    async with session.get(url=url, timeout=ClientTimeout(total=5)) as r:
    ...
    async with session.get(url=audioUrl,
        ...
        with open(f"{guild}_{botid}.mp3", 'wb') as f:
            while True:
...
def start_play(guild: str, port: dict, botid: str):
    return Popen(
        f'ffmpeg -re -nostats -i "{guild}_{botid}.mp3" -acodec libopus -ab 128k -f mpegts zmq:tcp://127.0.0.1:{port[guild]}',
        shell=True,
        encoding='utf-8',
    )
...
def get_playlist(guild: str, playlist: dict):
    ...
    for item in playlist[guild]:
        ...
        i += 1
    c.append(Module.Header(f'共有{len(playlist[guild])}首歌'))
    return c
...
async def delmsg(msg_id: str, config: dict, botid: str, session: ClientSession,
    ...
    data = {"msg_id": msg_id}
    headers = {"Authorization": f"Bot {config[f'token{botid}']}"}
...
def load_config():
    try:
        with open("config.json", "r", encoding="utf-8") as f:
            ...
            return {k: v.replace('!', '\\') for k, v in configtmp.items()}
    except:
...
async def login(bot: Bot, botid: str, qq_enable: str, netease_phone: str,
    ...
    async with ClientSession(connector=TCPConnector(ssl=False)) as session:
        ...
        try:
            ...
            if response == -10:
                url = f'http://127.0.0.1:3000/login/cellphone?phone={netease_phone}&password={netease_passwd}'
                async with session.get(url=url,
                                       timeout=ClientTimeout(total=5)) as r:
                    logger.warning(await r.text())
        ...
        if qq_enable == "1":
            ...
            async with session.post(url=url,
                                    ...
                                    timeout=ClientTimeout(total=5)) as r:
                logger.warning(await r.text())
            url = f'http://127.0.0.1:3300/user/getCookie?id={qq_id}'
...
async def save_cache(botid: str, playlist: dict, voicechannelid: dict,
    ...
    with open(f'{botid}playlistcache', 'w', encoding='utf-8') as f:
        f.write(str(playlist))
    with open(f'{botid}voiceidcache', 'w', encoding='utf-8') as f:
        f.write(str(voicechannelid))
    with open(f'{botid}msgidcache', 'w', encoding='utf-8') as f:
        f.write(str(msgid))
    with open(f'{botid}channelidcache', 'w', encoding='utf-8') as f:
        tmp = {guild: chl.id for guild, chl in channel.items()}
        f.write(str(tmp))
...
async def disconnect(bot: Bot, guild: str, voice: dict, timeout: dict,
    ...
    logger.warning(f"{guild} disconnected")
...
async def async_run_in_executor(f, *args, loop: AbstractEventLoop,
    ...
    try:
        return await loop.run_in_executor(
            executor, partial(f, *args, **kwargs)
        )
